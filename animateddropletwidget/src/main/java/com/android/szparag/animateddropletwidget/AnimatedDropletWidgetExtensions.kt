...
fun lerp(first: Long, second: Long, factor: Float) =
  (first + factor * (second - first)).toLong()

fun lerp(first: Int, second: Int, factor: Float) =
  (first + factor * (second - first)).toInt()

fun inverseLerp(first: Int, second: Int, actual: Float) =
  (actual.clamp(
    Math.max(first, second).toFloat(), Math.min(first, second).toFloat()) - first) / (second - first)
...
fun Animation.setListenerBy(
  onStart: (Animation?) -> (Unit) = animationListenerCallbackStub, onEnd: (Animation?) -> (Unit) = animationListenerCallbackStub,
...
fun View.asString() =
  "${asShortString()} (${id.toResourceEntryName(context)})"

fun Drawable.asString() =
  if (VERSION.SDK_INT >= VERSION_CODES.KITKAT) asStringPostKitkat() else asStringPreKitkat()

@RequiresApi(VERSION_CODES.KITKAT) fun Drawable.asStringPostKitkat() =
...
fun Any.asShortString() =
  "${this::class.java.simpleName}@${hashCode()}"
...
fun View.setSize(size: Pair<Width, Height>) =
...
fun LayoutParams.asString() =
  "${asShortString()}, (width, height): ($width, $height)"
